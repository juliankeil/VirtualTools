function [s,cfg] = vt_statfun_depsamplesGLM(cfg, dat, design)
...
% compute the statistic
    ...
    cond_vec=design(cfg.ivar:cfg.cvar,:);
    if length(cond_vec) ~= size(dat,2)
    ...
    for d=1:size(dat,1)
        ...
        tmp = (dat(d,:)-min(dat(d,:)))./(max(dat(d,:))-min(dat(d,:))); % Scale between 0 and 1 before norminv
        % Avoid -Inf
        tmp(tmp == 1) = 0.9999;
        tmp(tmp == 0) = 0.0001;
        tmp = norminv(tmp);
        % Compute the Linear Model
        mod1 = fitlm(cond_vec',tmp','interactions','CategoricalVars',1:2);

        % 3.2.2. Extract the Beta Coefficients
        b(d,1) = mod1.Coefficients{1,1}; % Intercept -> ERP acros conditions                
        b(d,2) = mod1.Coefficients{2,1}; % First Column: Auditory vs. Visual
        b(d,3) = mod1.Coefficients{3,1}; % Second Column: Real vs. Fake
        b(d,4) = mod1.Coefficients{4,1}; % Interaction
        
        p(d,1) = mod1.Coefficients{1,4}; % Intercept -> ERP acros conditions                
        p(d,2) = mod1.Coefficients{2,4}; % First Column: Auditory vs. Visual
        p(d,3) = mod1.Coefficients{3,4}; % Second Column: Real vs. Fake
        p(d,4) = mod1.Coefficients{4,4}; % Interaction
    end
    close(h)
    for l = 1:size(b,2)
        s.stat(l,:,:) = reshape(b(:,l),cfg.dim);
        s.prob(l,:,:) = reshape(p(:,l),cfg.dim);
    end
end
